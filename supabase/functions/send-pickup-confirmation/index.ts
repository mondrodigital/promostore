...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Received body for pickup confirmation:', orderData);
  ...
  try {
    const { data: settingsData, error: settingsError } = await supabaseAdmin
      ...
      .eq('template_id', 'picked_up_confirmation')
      .single()

    if (settingsError || !settingsData) {
      console.error('Error fetching picked_up_confirmation settings:', settingsError)
      return new Response(JSON.stringify({ message: 'Could not load email settings.' }), { status: 500, headers: baseHeaders });
    }

    const { subject: subjectTemplate, body_html: bodyTemplate } = settingsData;
    if (!subjectTemplate || !bodyTemplate) {
        return new Response(JSON.stringify({ message: 'Missing required fields in picked_up_confirmation settings.' }), { status: 500, headers: baseHeaders });
    }

    const itemsHtml = orderData.items?.length > 0
      ? `<ul>${orderData.items.map((item: any) => `<li>${item.name} (Qty: ${item.quantity})</li>`).join('')}</ul>`
      : '<p>(Item details not provided)</p>';
    ...
    const replacements = {
      ...
      '{returnDate}': orderData.returnDate || 'N/A',
      '{itemsHtml}': itemsHtml,
    };
    ...
    console.log('Pickup confirmation email sent successfully:', data)
    return new Response(JSON.stringify({ message: 'Pickup confirmation email sent successfully', data }), { headers: baseHeaders, status: 200 })
