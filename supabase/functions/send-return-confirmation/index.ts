...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Received body for return confirmation:', orderData);
  ...
  try {
    const { data: settingsData, error: settingsError } = await supabaseAdmin
      ...
      .eq('template_id', 'returned_confirmation')
      .single()

    if (settingsError || !settingsData) {
      console.error('Error fetching returned_confirmation settings:', settingsError)
    ...
    if (!subjectTemplate || !bodyTemplate) {
      return new Response(JSON.stringify({ message: 'Missing required fields in returned_confirmation settings.' }), { 
        status: 500, 
        headers: baseHeaders 
      });
    }
    ...
    console.log('Return confirmation email sent successfully:', data)
    return new Response(JSON.stringify({ message: 'Return confirmation email sent successfully', data }), { headers: baseHeaders, status: 200 })
