...
serve(async (req)=>{
  ...
  const supabaseAdmin = createClient(Deno.env.get('PROJECT_SUPABASE_URL') ?? '', Deno.env.get('PROJECT_SUPABASE_SERVICE_ROLE_KEY') ?? '');
  ...
  try {
    ...
    const { data: settingsData, error: settingsError } = await supabaseAdmin.from('email_settings').select('subject, body_html').eq('template_id', 'internal_notification').single();
    ...
    if (!subjectTemplate || !bodyTemplate) {
      // Use the Headers object
      return new Response(JSON.stringify({
        message: 'Missing required fields in order notification settings.'
      }), {
        status: 500,
        headers: baseHeaders
      });
    ...
    for (const [key, value] of Object.entries(replacements)){
      ...
      const singleBraceKey = key.replace('{{', '{').replace('}}', '}');
      const escapedSingleKey = singleBraceKey.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      ...
      subject = subject.replace(new RegExp(escapedSingleKey, 'g'), replacementValue);
      bodyHtml = bodyHtml.replace(new RegExp(escapedKey, 'g'), replacementValue);
      bodyHtml = bodyHtml.replace(new RegExp(escapedSingleKey, 'g'), replacementValue);
    }
