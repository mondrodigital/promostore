...
CREATE OR REPLACE FUNCTION validate_vellum_email(email text)
RETURNS boolean
LANGUAGE plpgsql
AS $$
BEGIN
  RETURN email ILIKE '%@vellummortgage.com';
END;
$$;

-- Update the order creation function to validate email
...
BEGIN
  ...
  IF NOT validate_vellum_email(p_user_email) THEN
    RAISE EXCEPTION 'Invalid email domain. Please use your @vellummortgage.com email address.';
  END IF;
