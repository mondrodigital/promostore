...
import { Search, Filter, Trash2, RefreshCw, ChevronDown, ChevronLeft, ChevronRight, AlertTriangle, Hourglass } from 'lucide-react';
...
export default function OrdersView({
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter(order => {
      // Notification-issues filter chip (C: workstream notification reliability)
      ...
      // Expired-wishlist filter chip (C)
      ...
      // Client-side search (status filter is handled server-side via pagination)
      if (searchQuery.trim()) {
        ...
        if (!matchesName && !matchesEmail && !matchesOrderNumber) return false;
      }

      return true;
    });
  }, [orders, searchQuery, showNotificationIssuesOnly, showExpiredWishlistsOnly]);
